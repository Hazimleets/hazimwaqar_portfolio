...
/* ─────────────────────────────────────────────
   DATA — 60+ technologies, 9 categories
───────────────────────────────────────────── */
const TECH_GRID = [
  // ── Languages ──────────────────────────────
  ...
  { name: "Cursor AI",        color: "#000000", cat: "Tools & Frameworks", abbr: "CUR"  },
  ...
  { name: "Next.js",          color: "#000000", cat: "Frontend",         abbr: "NXT"  },
...
/* category colour map — used for coloured filter pills */
const CAT_COLORS = {
  "Languages":           "#3776AB",
...
const TECH_STACK = ["Python","TensorFlow","PyTorch","LangChain","Streamlit","React","Docker","FastAPI","MySQL"];

const PROJECTS = [
  {
    title: "AI-Powered Proposal & Pitch Deck Generator",
    desc: "Automated system that generates structured business proposals and pitch decks from user prompts using large language models.",
  ...
  {
    title: "Smart Voice & Portfolio Chat Assistant",
    desc: "Voice-enabled chatbot using Whisper AI for transcription and LLaMA for response generation.",
    tags: ["Whisper AI","LLaMA","React"],
    href: "#",
  },
  {
    title: "MagicMorph: Prompt or Photo to Art",
    desc: "A creative AI pipeline converting text prompts or uploaded photos into stylised artwork using diffusion models.",
    tags: ["Stable Diffusion","Python","GenAI"],
    href: "#",
  },
  {
    title: "Fine-Tuning Deep Learning Models",
    desc: "Explored PEFT techniques to fine-tune DeepSeek math and domain-specific LLMs for improved task performance.",
    tags: ["PyTorch","DeepSeek","PEFT"],
    href: "#",
  },
  {
    title: "AI Reader: Semantic QA over Enterprise Docs",
    desc: "RAG-based document intelligence system enabling natural language querying over large enterprise document repositories.",
...
/* ─────────────────────────────────────────────
   TECH STACK GRID (collapsible categories)
───────────────────────────────────────────── */
      ...
      {openCategory && (
        <div style={{ 
          ...
          <div style={{ 
            ...
            <span style={{ width: 7, height: 7, borderRadius: "50%", background: CAT_COLORS[openCategory], flexShrink: 0 }} />
            <span style={{ fontSize: "0.72rem", fontWeight: 700, color: CAT_COLORS[openCategory] }}>{openCategory}</span>
            <span style={{ fontSize: "0.7rem", color: "var(--muted)" }}>
              — {TECH_GRID.filter(t => t.cat === openCategory).length} technologies
            </span>
            <button 
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(78px, 1fr))", gap: "0.55rem" }}>
            {TECH_GRID.filter(t => t.cat === openCategory).map(item => (
              <TechCard key={item.name + item.cat} item={item} />
            ))}
          </div>
        </div>
      )}
      ...
      {!openCategory && (
        <div style={{ 
          ...
          <p style={{ fontSize: "0.72rem", color: "var(--muted)" }}>
            Click a category above to explore <strong style={{ color: "var(--purple)" }}>{TECH_GRID.length}</strong> technologies
          </p>
        </div>
      )}
    </div>
  ...
  return (
    <a
      ...
      style={{
        ...
        transition: `opacity 0.6s ease ${index * 0.1}s, transform 0.4s ease, border-color 0.25s, box-shadow 0.3s`,
        boxShadow: hovered ? "0 20px 60px rgba(0,0,0,0.5)" : "none",
      }}
    >
      <div style={{ position: "relative", height: 200, background: "linear-gradient(135deg,#0f1a3a 0%,#1a0f3a 100%)", overflow: "hidden" }}>
        <div style={{ width: "100%", height: "100%", display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", gap: "0.5rem" }}>
          <svg width="52" height="52" viewBox="0 0 52 52" fill="none">
            <rect width="52" height="52" rx="14" fill="rgba(255,255,255,0.05)" />
            <rect x="1" y="1" width="50" height="50" rx="13" stroke="rgba(255,255,255,0.08)" />
            <path d="M14 38L22 24L29 34L33 26L38 38H14Z" stroke="rgba(255,255,255,0.25)" strokeWidth="1.5" fill="none" strokeLinejoin="round" />
            <circle cx="33" cy="18" r="5" stroke="rgba(255,255,255,0.25)" strokeWidth="1.5" fill="none" />
          </svg>
          <span style={{ fontSize: "0.68rem", color: "rgba(255,255,255,0.18)", fontStyle: "italic" }}>replace with project screenshot</span>
        </div>
        <div style={{
          ...
          <span style={{ background: "var(--purple)", color: "#000", padding: "0.55rem 1.3rem", borderRadius: 8, fontWeight: 700, fontSize: "0.85rem" }}>
            Check Live Site ↗
          </span>
        </div>
      </div>
      <div style={{ padding: "1.5rem" }}>
        <h3 style={{ fontWeight: 800, color: "var(--white)", fontSize: "1rem", lineHeight: 1.4, marginBottom: "0.6rem", letterSpacing: "-0.01em" }}>
          {project.title}
        </h3>
        ...
        <div style={{ display: "flex", flexWrap: "wrap", gap: "0.4rem" }}>
          {project.tags.map((t) => (
            <span key={t} style={{ background: "rgba(255,255,255,0.05)", border: "1px solid rgba(255,255,255,0.1)", borderRadius: 6, padding: "0.2rem 0.65rem", fontSize: "0.72rem", fontWeight: 600, color: "var(--off)", letterSpacing: "0.04em" }}>
              {t}
            </span>
          ))}
        </div>
...
const Icons = {
  layers: (
    <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="var(--purple)" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
    ...
    <>
      ...
      <nav style={{
        ...
        <div style={{ fontWeight: 800, fontSize: "1.2rem", color: "var(--white)", letterSpacing: "-0.03em", cursor: "pointer" }} onClick={() => scrollTo("home")}>
          <span style={{ background: "var(--grad)", WebkitBackgroundClip: "text", WebkitTextFillColor: "transparent" }}>Hazim</span>
          {" Waqar"}
        </div>
          ...
          {NAV_ITEMS.map((n) => (
            <li key={n.id}>
              <button onClick={() => scrollTo(n.id)} style={{
                ...
                padding: "0.45rem 0.95rem", borderRadius: 7,
        ...
        <button
          ...
          style={{ ...btnPrimary, padding: "0.55rem 1.3rem", fontSize: "0.84rem" }}
        >
          Book a Call
        </button>
        ...
        >
          <span style={{ width: 22, height: 1.5, background: "var(--white)", display: "block", borderRadius: 2 }} />
          <span style={{ width: 22, height: 1.5, background: "var(--white)", display: "block", borderRadius: 2 }} />
          <span style={{ width: 22, height: 1.5, background: "var(--white)", display: "block", borderRadius: 2 }} />
        </button>
      </nav>
        ...
        <div style={{
          ...
          {NAV_ITEMS.map((n) => (
            <button key={n.id} onClick={() => scrollTo(n.id)} style={{
              ...
              color: "var(--off)", padding: "0.65rem 0.4rem",
              borderBottom: "1px solid var(--border)",
            }}>
              {n.label}
            </button>
          ...
          <div className="hero-grid" style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "4rem", alignItems: "center" }}>
            ...
            <FadeIn>
              ...
              <p style={{ fontSize: "1.05rem", color: "var(--muted)", marginBottom: "2rem", lineHeight: 1.7 }}>
                ...
                {", a "}
                <TypingText words={["AI Engineer", "ML Researcher", "NLP Specialist", "GenAI Developer", "MLOps Engineer"]} />
                {" based in Pakistan"}
              </p>

              <div style={{ display: "flex", gap: "1rem", flexWrap: "wrap" }}>
                <button onClick={() => scrollTo("portfolio")} style={btnPrimary}>Show my Work</button>
                <a href="/Hazim-Resume.pdf" target="_blank" rel="noreferrer" style={btnGhost}>Download CV</a>
              </div>
            </FadeIn>
        ...
        <div className="container">
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit,minmax(290px,1fr))", gap: "1.25rem" }}>
            {[
              ...
              { title: "Agentic AI Development",  icon: Icons.brain,   desc: "Autonomous AI agents capable of planning, reasoning, tool usage, memory management, and multi-step task execution using frameworks like LangChain, AutoGen, CrewAI, and MCP (Model Context Protocol) for tool and system integration." },
            ].map((s, i) => (
        ...
        <div className="container">
          ...
          <FadeIn>
            <div style={{ marginTop: "4rem", background: "linear-gradient(135deg,rgba(79,172,254,0.08),rgba(203,172,249,0.08))", border: "1px solid rgba(203,172,249,0.15)", borderRadius: 24, padding: "3rem 2.5rem", textAlign: "center" }}>
              ...
              <button onClick={() => scrollTo("contact")} style={btnPrimary}>
                {"Let's get in touch"}
              </button>
            </div>
          </FadeIn>
